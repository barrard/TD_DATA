let { get_minute_data, request_historical_data, request_all_movers } = require("./request_historical_data.js");
let { bars_to_csv, get_start_end_dates, get_most_recent_day } = require("./utils/transform.js");
let { save_data, open_file, save_fd } = require("./utils/files.js");
let Symbols = require("./utils/symbols.js");
const minutley_data_path = symbol => `./TD_data/${symbol}/${symbol}_minutely.csv`;
...
async function forward_fill_minutely_data(symbol){
  ...
  let { csv_file_data, appendable_fd } = await open_file(minutley_data_path(symbol));
...
async function get_historical_daily_data(symbol){
    ...
    let { csv_file_data, appendable_fd } = await open_file(daily_data_path(symbol));
    var bars;
    if (!csv_file_data) {
